...
const FacultyDashboard = () => {
  ...
  useEffect(() => {
    if (!accessToken || !userId || !isStaff) {
      toast.error(isStaff ? 'Please log in.' : 'Access restricted to faculty.', { position: 'top-right' });
      navigate('/login');
      return;
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const userResources = resources.filter((resource) => resource.staff === userId);
        setDashboardData((prev) => ({
          ...prev,
          sharedResources: userResources.length,
          recentResources: userResources
            .sort((a, b) => new Date(b.created_on) - new Date(a.created_on))
            .slice(0, 3),
        }));
      } catch (error) {
        toast.error('Failed to fetch shared resources', { position: 'top-right' });
      } finally {
    ...
    fetchData();
  }, [accessToken, navigate, userId, isStaff]);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="welcome-container animate-fade-in mb-8">
        ...
        <div className="md:col-span-3">
          <div className="grid gap-4 md:grid-cols-5 grid-cols-2 mb-8">
            ...
            <div className="bg-indigo-500 p-4 text-white rounded flex items-center">
              ...
              <div>
                <p className="text-sm">Shared Resources</p>
                <p className="text-2xl font-bold">
                  {loading.resources ? '...' : dashboardData.sharedResources}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 contact-form">
                  ...
                  {dashboardData.participatableEvents.slice(0, 3).map((event) => {
                    ...
                    return (
                      <li key={event.id} className="border-b pb-2">
                        <p>
                          <span className="font-semibold mr-1">Event:</span>
                          {event.title}
                        </p>
            ...
            <div className="bg-white p-6 rounded shadow contact-form">
              <h3 className="text-xl font-bold mb-4 flex items-center">
                <FontAwesomeIcon icon={faFileAlt} className="mr-2" />
                Resources Shared by You ({loading.resources ? '...' : dashboardData.sharedResources})
              </h3>
              {loading.resources ? (
                ...
                <ul className="space-y-4">
                  {dashboardData.recentResources.map((resource) => (
                    <li key={resource.id} className="border-b pb-2">
                      ...
                      <p className="text-sm text-gray-600">
                        Shared on: {format(new Date(resource.created_on), 'MMM d, yyyy')}
                      </p>
                    </li>
                  ))}
                </ul>
          ...
          <div className="mt-8 bg-white p-6 rounded shadow contact-form">
            <h3 className="text-xl font-bold mb-4 flex items-center">
              <FontAwesomeIcon icon={faFileAlt} className="mr-2" />
              Manage Resources
            </h3>
            <p className="mb-4">Share course materials, lecture notes, or other resources with your students.</p>
            <Link
              to="/resource-share"
              state={{ user, profile }}
